package com.cabify.challenge.core.type
...
sealed class Either<out L, out R> {
  ...
  inline fun <C, P> bimap(left: (L) -> C, right: (R) -> P): Either<C, P> = fold({
    Left(
      left(it)
    )
  }, { Right(right(it)) })
...
fun <L, R, S> Either<L, R>.flatMap(f: (R) -> Either<L, S>): Either<L, S> = fold({
  Either.Left(it)
}, { f(it) })
